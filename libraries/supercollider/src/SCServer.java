...
public class SCServer
{
	...
	public static SCServer local;
	public SCGroup default_group;
	public SCGroup root_group;
	...
	public SCBuffer [] buffers;

	// Class init
	static
	{
		SCServer.local = new SCServer("127.0.0.1", 57110); 
	}
	...
	public SCServer(String server_ip, int server_port)
	{
		...
		buffers = new SCBuffer[512];
		
		// Need to allocate a global OSC listener.
		if (SCServer.osc == null)
		{
			SCServer.osc = new OscP5(this, 57150);
			
			// SRSP needed to receive aynschronous messages on the same port
			SCServer.osc.properties().setSRSP(OscProperties.ON);
		}
		
		root_group = new SCGroup(this, 0);
		default_group = new SCGroup(this, 1);
	...
	public void freeAll ()
	{
		...
		msg.add(0);
		SCServer.osc.send(msg, addr);
	}
	
	// this.addResponder("/n_set", "isf", "handleGet");
